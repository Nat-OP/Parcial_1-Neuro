...
const float THR_ROLL = 15.0f, THR_PITCH = 15.0f, GRADOS_GIRO = 45.0f, DT = 10.0f;
...
void modoCauteloso() {
    W_EVA=0.85f; W_ATR=0.15f; W_TERRENO=10.0f; W_MEM_OSC=0.75f; W_RETRO=0.20f;
...
void modoTemerario() {
    W_EVA=0.25f; W_ATR=0.75f; W_TERRENO=0.01f; W_MEM_OSC=0.3f; W_RETRO=0.08f;
...
void setup() {
	...
	I2C_IMU.setClock(400000);
	I2C_IMU.setTimeOut(10); // <--- IMPORTANTE: Si la IMU no responde en 10ms, sigue con el loop
...
// Variable label declarada aquí para que sea visible en todo el loop
	...
	if (millis() - lastTel > 100) { // Enviar máximo 10 veces por segundo
	    lastTel = millis();
	    
	    if (xSemaphoreTake(telMutex, pdMS_TO_TICKS(5)) == pdTRUE) { // Intentar tomar la llave (max 5ms)
		snprintf(telBuf, sizeof(telBuf), "ST:%s,DF:%.1f,OBJ:%.1f,R:%.1f,P:%.1f,Y:%.1f...\n", 
		         label, distFront, distObj, roll, pitch, yaw); // (Asegúrate que coincidan las variables)
		
		// Enviamos el dato inmediatamente para no guardarlo en buffers internos largos
		Serial.print(telBuf); 
		
		xSemaphoreGive(telMutex); // DEVOLVER SIEMPRE LA LLAVE
	    }
	}
