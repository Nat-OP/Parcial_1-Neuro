...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .cmd-btn {
    ...
    color: var(--accent); font-family: 'IBM Plex Mono'; font-weight: 600;
    cursor: pointer; transition: 0.2s; text-transform: uppercase; font-size: 11px;
  }
  .cmd-btn:hover { background: var(--border); }
  .cmd-btn:active { background: var(--accent); color: var(--bg); }
  ...
  .log-line { border-bottom: 1px solid #0d1218; }
  .log-line .lts  { color: #3a4a5a; margin-right: 6px; }
  .log-line .ltag { color: #1a6a3a; margin-right: 4px; }
  .log-line .ldat { color: #5a7080; }
...
<div class="cmd-bar">
  <button class="cmd-btn" onclick="sendCmd('C')">Crucero [C]</button>
  <button class="cmd-btn" onclick="sendCmd('E')">Evitación [E]</button>
  <button class="cmd-btn" onclick="sendCmd('T')">Target [T]</button>
</div>
...
function redrawAll() {
  drawChart('chart-n', ['N0','N1','N2','N3','N4','N5','N6','MEM'], ['#4477dd','#1D9E75','#0F6E56','#e03030','#c47a00','#8877ee','#dd5588','#4a9e20'], 100);
  drawChart('chart-k', ['N3','ATR','ARB'], ['#e03030','#00d4ff','#ff8c00'], 100);
}
...
function connect() {
  ...
  ws.onopen = () => { badge.textContent = '● CONECTADO'; badge.className = 'connected'; wsRetry = 0; };
  ws.onclose = () => { badge.textContent = '● DESCONECTADO'; badge.className = ''; wsRetry++; setTimeout(connect, Math.min(wsRetry * 1000, 5000)); };
  ws.onerror = () => { badge.textContent = '● ERROR WS'; badge.className = 'error'; };
  ws.onmessage = (ev) => {
    const raw = ev.data;
    if (raw.startsWith('ST:')) applyData(parseCSV(raw));
    else addLog(raw);
  };
}
...
void setup() {
    ...
    // REDIRECCIÓN DE WEBSOCKET A SERIAL1
    webServer.onEvent([](uint8_t num, WStype_t type, uint8_t* payload, size_t length) {
        ...
        if (type == WStype_TEXT) {
            // Enviamos el carácter recibido ('C', 'E' o 'T') a la ESP Grande
            Serial1.write(payload, length);
            Serial.printf("[CMD] Enviado a Cerebro: %s\n", payload);
        }
