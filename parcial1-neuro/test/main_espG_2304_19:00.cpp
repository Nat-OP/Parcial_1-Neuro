...
const int   PWM_CRUCERO   = 60;   // Driver 1 (M1, M2)
const int   PWM_CRUCERO_2 = 100;   // Driver 2 (M3, M4) — ponderado mayor
const int   PWM_GIRO      = 100;
const int   PWM_GIRO_2    = 140;
const float GRADOS_GIRO   = 45.0f;
const float THR_ROLL      =  95.0f;
const float THR_PITCH     =  95.0f;
...
void setMotores(int vI, int vD) {
    ...
    // Driver 2 — escala el PWM proporcionalmente al ponderado
    int vI2 = (vI == 0) ? 0 : (vI > 0 ? PWM_CRUCERO_2 : -PWM_CRUCERO_2);
    int vD2 = (vD == 0) ? 0 : (vD > 0 ? PWM_CRUCERO_2 : -PWM_CRUCERO_2);
    setMotor(CH_M1, M1_A, M1_B,  vI2);
    setMotor(CH_M4, M4_A, M4_B,  vD2);
}
